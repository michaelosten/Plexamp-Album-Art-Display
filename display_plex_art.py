...
PLEX_TOKEN = os.getenv('PLEX_TOKEN', 'quPN1No3CxVxksWm9dmm')
...
def start_x_server():
    if not os.environ.get('DISPLAY'):
        logging.info("Starting X server...")
        try:
            subprocess.run(['startx'], check=True)
        except subprocess.CalledProcessError as e:
            logging.error(f"Error starting X server: {e}")
            return False
        except FileNotFoundError:
            logging.error("startx command not found. Make sure X server is installed.")
            return False
        except Exception as e:
            logging.error(f"Unexpected error starting X server: {e}")
            return False
        
        time.sleep(5)
        os.environ['DISPLAY'] = ':0'
        logging.info("X server started and DISPLAY set to :0")
    return True
...
def get_plex_server_public_address():
    ...
    try:
        ...
        logging.debug("Response text from Plex.tv API: %s", response.text)
        ...
        logging.error(f"Failed to get Plex server public address: {e}")
        return None
...
def fetch_and_display_cover_art(metadata):
    if not metadata:
        logging.info("Nothing Playing Currently")
    ...
    if cover_art_url:
        logging.info(f"Cover art URL: {cover_art_url}")
        try:
            ...
            logging.error(f"Failed to fetch cover art: {e}")
            display_image_not_available()
    else:
        logging.info("No cover art found")
        display_image_not_available()

def display_with_feh(image_path):
    logging.info(f"Attempting to display image with feh: {image_path}")
    try:
        subprocess.Popen(['feh', '-F', '-Z', image_path])
    except Exception as e:
        logging.error(f"Can't Display: {e}")

def display_image_not_available():
    logging.info("Displaying 'Image Not Available' message")
    message = "Image Not Available"
    img = Image.new('RGB', (800, 600), color='black')
    d = ImageDraw.Draw(img)
    fnt = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', 40)
    bbox = d.textbbox((0, 0), message, font=fnt)
    w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
    d.text(((800-w)/2, (600-h)/2), message, font=fnt, fill=(255, 255, 255))

    message_image_path = '/tmp/image_not_available.jpg'
...
def check_currently_playing():
    ...
    while not stop_thread:
        logging.info("Checking for currently playing media...")
        ...
        if metadata:
            ...
            if new_cover_art_url != current_cover_art_url:
                logging.info(f"New cover art URL detected: {new_cover_art_url}")
                current_cover_art_url = new_cover_art_url
                fetch_and_display_cover_art(metadata)
            else:
                logging.info("Cover art URL has not changed.")
        else:
        ...
        for remaining in range(15, 0, -1):
            if stop_thread:
                break
            logging.info("Next fetch in %d seconds...", remaining)
            time.sleep(1)
...
def main():
    ...
    if not PLEX_SERVER_BASE_URL:
        logging.error("Could not get Plex server public address.")
        return
    
    if not start_x_server():
        logging.error("Unable to start X server. Exiting.")
        return
